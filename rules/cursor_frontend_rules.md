...
## SEO и микроразметка Schema.org

### Правила использования ItemList

**❌ Ошибки:**

- `position` нельзя задавать текстом "01", "02" — это должно быть число
- Нельзя использовать `itemprop="position"` на визуальном элементе с текстом
...
>
    ...
    <p itemprop="name">Текст элемента списка</p>
</div>
```

**Правила:**

1. `position` — всегда число через `<meta itemprop="position" content="1">`
2. Визуальный номер ("01", "02") — отдельный элемент без микроразметки
3. `name` — на текстовом элементе с контентом
4. `item` в `ListItem` — только если есть ссылка на конкретный объект (необязательно для простых списков)

### Общие правила SEO

- Семантические теги: `header`, `section`, `article`, `aside`, `nav`
...
- [ ] Контекст ≤ 1 уровень.
- [ ] Медиа внизу файла.
- [ ] Микроразметка Schema.org корректна (position — число через meta).
